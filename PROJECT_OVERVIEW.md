...
- ✅ `/api/payments/131/sbp/orders/:id` - статус платежа СБП
- ✅ `/api/payments/131/webhook` - вебхуки Банка 131
...
3. 💳 Приём оплат через СБП (Банк 131)
...
2. **Банк 131 (СБП)**
   - Активировать проект и получить `project`, `merchant`, `keyId`
   - Сгенерировать RSA ключи
   - Добавить `PAYMENT_131_*` переменные в `.env`
   - Настроить webhook `/api/payments/131/webhook`
