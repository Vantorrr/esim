# 🌊 eWave - Telegram Mini App для продажи eSIM

Современное веб-приложение для автоматической продажи eSIM через Telegram Mini Apps с интеграцией API esim-go.com.

## ✨ Возможности

- 🌍 **200+ стран** - покрытие по всему миру
- ⚡ **Мгновенная активация** - получите eSIM за 1 минуту
- 💳 **Удобная оплата** - Stripe и ЮKassa
- 📱 **QR-код** - простая установка через сканирование
- 🎨 **Современный UI** - красивый интерфейс с градиентами
- 🌐 **Мультиязычность** - поддержка RU/EN
- 🔒 **Безопасность** - защищённые платежи PCI DSS

## 🛠️ Технологии

### Frontend
- **Next.js 14** - React фреймворк
- **TypeScript** - типизация
- **Tailwind CSS** - стилизация
- **Telegram Mini Apps SDK** - интеграция с Telegram

### Backend
- **Node.js + Express** - API сервер
- **esim-go API** - провайдер eSIM
- **Stripe** - международные платежи
- **Т-Банк (Tinkoff)** - платежи РФ, СБП, рассрочка
- **YooKassa** - платежи РФ

## 🚀 Быстрый старт

### 1. Установка зависимостей

\`\`\`bash
npm install
\`\`\`

### 2. Настройка переменных окружения

Создайте файл `.env` в корне проекта:

\`\`\`env
# Telegram Bot
TELEGRAM_BOT_TOKEN=your_bot_token_here

# eSIM-GO API
ESIM_GO_API_KEY=your_esim_go_api_key
ESIM_GO_API_URL=https://api.esim-go.com/v2.2

# Payment Systems
STRIPE_SECRET_KEY=sk_test_...
STRIPE_PUBLISHABLE_KEY=pk_test_...
STRIPE_WEBHOOK_SECRET=whsec_...

YOOKASSA_SHOP_ID=your_shop_id
YOOKASSA_SECRET_KEY=your_secret_key

# Server
PORT=8080
NODE_ENV=development
NEXT_PUBLIC_API_URL=http://localhost:8080/api
WEBHOOK_URL=https://your-domain.com/api/webhook
\`\`\`

### 3. Получение API ключей

#### esim-go.com
1. Зарегистрируйтесь на https://esim-go.com
2. Перейдите в раздел API
3. Создайте новый API ключ
4. Скопируйте ключ в `.env`

#### Stripe
1. Создайте аккаунт на https://stripe.com
2. Перейдите в Developers → API keys
3. Скопируйте Secret key и Publishable key
4. Настройте webhook endpoint для получения событий

#### Т-Банк (Tinkoff)
1. Подключите эквайринг на https://www.tbank.ru/business/ecom/
2. Создайте терминал в личном кабинете
3. Получите Terminal Key и Password
4. Подробная инструкция: **TINKOFF_SETUP.md**

#### YooKassa
1. Зарегистрируйтесь на https://yookassa.ru
2. Получите Shop ID и Secret Key
3. Настройте уведомления о платежах

### 4. Запуск в режиме разработки

\`\`\`bash
# Запуск backend сервера
npm run server:dev

# В другом терминале - запуск frontend
npm run dev
\`\`\`

Приложение будет доступно:
- **Frontend**: http://localhost:3000
- **Backend**: http://localhost:8080

### 5. Создание Telegram бота

1. Напишите [@BotFather](https://t.me/BotFather) в Telegram
2. Создайте нового бота командой `/newbot`
3. Получите токен и добавьте в `.env`
4. Настройте Mini App:
   \`\`\`
   /newapp
   /setdomain - укажите ваш домен
   /setdescription - добавьте описание
   /setimage - загрузите иконку
   \`\`\`

## 📦 Структура проекта

\`\`\`
eWave/
├── app/                    # Next.js app directory
│   ├── page.tsx           # Главная страница
│   ├── checkout/          # Страница оплаты
│   ├── success/           # Страница успешной покупки
│   ├── faq/              # FAQ
│   ├── how-to-install/   # Инструкция по установке
│   ├── compatibility/    # Совместимость устройств
│   ├── layout.tsx        # Главный layout
│   └── globals.css       # Глобальные стили
├── components/            # React компоненты
│   ├── Header.tsx
│   ├── Footer.tsx
│   ├── CountrySelector.tsx
│   ├── PackageList.tsx
│   ├── PackageCard.tsx
│   └── QRCodeDisplay.tsx
├── lib/                   # Утилиты и хелперы
│   ├── telegram.ts       # Telegram SDK
│   └── api.ts           # API клиент
├── server/               # Backend
│   ├── index.js         # Express сервер
│   ├── routes/          # API роуты
│   │   ├── esim.js
│   │   ├── payment.js
│   │   └── webhook.js
│   └── services/        # Сервисы
│       └── esimGoApi.js
├── public/              # Статические файлы
├── .env                 # Переменные окружения
├── package.json
└── README.md
\`\`\`

## 🎨 Цветовая схема

Приложение использует современную цветовую палитру:

- **Primary** (Синий): `#0080FF` - основной цвет, градиенты
- **Secondary** (Оранжевый): `#FF8534` - акценты, кнопки
- **Background**: `#F0F7FF` - светло-голубой фон
- **Text Primary**: `#143E66` - тёмно-синий текст
- **Text Secondary**: `#2F5278` - вторичный текст

## 🔌 API Endpoints

### eSIM

\`\`\`
GET  /api/esim/countries          # Получить список стран
GET  /api/esim/packages            # Получить все пакеты
GET  /api/esim/packages?country=US # Пакеты для страны
GET  /api/esim/packages/:id        # Детали пакета
POST /api/esim/orders              # Создать заказ
GET  /api/esim/orders/:id          # Получить заказ
GET  /api/esim/orders/:id/qr       # Получить QR-код
\`\`\`

### Payment

\`\`\`
POST /api/payment/stripe/create-session    # Создать сессию Stripe
POST /api/tinkoff/create-payment           # Создать платёж Т-Банк
POST /api/payment/yookassa/create-payment  # Создать платёж YooKassa
\`\`\`

### Webhooks

\`\`\`
POST /api/webhook/stripe      # Webhook Stripe
POST /api/tinkoff/notification # Webhook Т-Банк
POST /api/webhook/yookassa    # Webhook YooKassa
\`\`\`

## 🌍 Развёртывание

### Vercel (Frontend)

\`\`\`bash
npm install -g vercel
vercel
\`\`\`

### Railway / Render / Heroku (Backend)

1. Подключите репозиторий
2. Добавьте переменные окружения
3. Установите команду запуска: `npm run server`
4. Укажите порт: `8080`

### Docker

\`\`\`bash
# Создайте Dockerfile если нужно
docker build -t ewave-app .
docker run -p 8080:8080 ewave-app
\`\`\`

## 📱 Тестирование в Telegram

1. Разверните приложение на публичном URL (можно использовать ngrok для тестирования)
2. Настройте Mini App в BotFather
3. Откройте бота в Telegram
4. Нажмите на кнопку меню для запуска Mini App

### Использование ngrok для локального тестирования:

\`\`\`bash
# Установите ngrok
npm install -g ngrok

# Запустите туннель
ngrok http 3000

# Используйте полученный URL в настройках бота
\`\`\`

## 💡 Советы по разработке

- Используйте `hapticFeedback()` для улучшения UX
- Telegram SDK автоматически применяет тему пользователя
- Тестируйте на реальных устройствах, а не только в браузере
- Backend должен быть на HTTPS для работы webhooks

## 🤝 Поддержка

- 📧 Email: support@ewave.com
- 💬 Telegram: [@support](https://t.me/support)
- 📚 Документация API: [esim-go.com/docs](https://docs.esim-go.com)

## 📄 Лицензия

MIT License - используйте как хотите!

## 🎯 TODO для production

- [ ] Добавить базу данных (PostgreSQL/MongoDB) для хранения заказов
- [ ] Реализовать систему уведомлений через Telegram Bot API
- [ ] Добавить кэширование тарифов (Redis)
- [ ] Настроить мониторинг и логирование (Sentry)
- [ ] Реализовать админ-панель для управления
- [ ] Добавить аналитику (Google Analytics / Amplitude)
- [ ] Настроить CI/CD pipeline
- [ ] Добавить rate limiting для API
- [ ] Реализовать систему рефералов
- [ ] Добавить поддержку большего количества платёжных систем

---

Сделано с ❤️ для путешественников по всему миру 🌍✈️
\`\`\`

